...
class ChampionModelScorer:
    """
    Khung gọi Champion Model (placeholder).

    Production: thay `score()` bằng gRPC/REST tới model serving hoặc ONNX runtime.
    Hiện tại: random risk_score 0–100; > threshold → fraud.
    """

    def __init__(self, fraud_threshold: int = FRAUD_THRESHOLD) -> None:
...
class FraudScoringMapper(MapFunction):
    ...
    def open(self, runtime_context) -> None:
        self._scorer = ChampionModelScorer()
